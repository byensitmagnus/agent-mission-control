...
| **0.2.0-candidate.16** | Development branch [`claude/candidate-16`](https://github.com/byensitmagnus/agent-mission-control/tree/claude/candidate-16); check [Releases](https://github.com/byensitmagnus/agent-mission-control/releases) for tagged artifacts. | A fixed report template with one `Status:` line and explicit `Delegation:` and `Review:` fields. [Haiku smoke](../evals/haiku-smoke-2026-09-25.md#candidate16-smoke-2026-09-26): case 08 passed 2/3, case 03 supplied a delegation reason in 2/3 (0/3 on candidate.15), and case 09 passed 1/1. One case 08 run still reported PASS without independent review. Runtime SHA-256 `267028bf…55b79fa34`. Independent read-only review on 2026-09-26 rejected the original R4 release claim; any publication is a limited report-format/workflow beta with advisory PASS. |
| **v0.2.0-candidate.15** | [Tagged ZIP](https://github.com/byensitmagnus/agent-mission-control/releases/tag/v0.2.0-candidate.15), with skill/plugin checksums. | Closes candidate.14's four deferred review points; a review the agent wrote itself does not count; final reports name delegation; `validate.py` enforces the size budget ([PRD](prd.md) R7). [Smoke](../evals/haiku-smoke-2026-09-25.md#candidate15-smoke-2026-09-26): the migration case passed in 1 of 3 Haiku runs, where one run started an independent reviewer on its own, and in the Sonnet run, whose reviewer found a real bug; the routing line was written in 0 of 3 Haiku runs of case 03. Released runtime SHA-256 `481288ca…75379a41`. |
| **v0.2.0-candidate.14** | [Tagged ZIP](https://github.com/byensitmagnus/agent-mission-control/releases/tag/v0.2.0-candidate.14), with skill/plugin checksums. | Release gate rebuilt from published practice ([sources](sources.md)): reuse an existing approval, otherwise one read-only reviewer or a person, one recheck after a fix, a table of excuses that do not change the verdict, and a `Review:` line in the final report. Not measured: no new model runs, so no behavior gain is claimed. Released runtime SHA-256 `231fa88e…6209d4c6`. |
| **v0.2.0-candidate.13** | [Tagged ZIP](https://github.com/byensitmagnus/agent-mission-control/releases/tag/v0.2.0-candidate.13), with skill/plugin checksums. | Consistency release: migration and data-loss risk route to `verification.md`, a missing required independent review keeps the verdict NOT VERIFIED, and `provenance.md` no longer cites unpublished sources as if public. No behavior gain claimed: Haiku case 08 held in 0 of 3 runs. Released runtime SHA-256 `4e6ece7a…0582f070`; the runs used `11ddc381…928f7daf`, which differs only in `references/provenance.md` wording. |
| **v0.2.0-candidate.12** | [Tagged ZIP](https://github.com/byensitmagnus/agent-mission-control/releases/tag/v0.2.0-candidate.12) from `9084bc0`, with skill/plugin checksums. | Moves the verdict rules small models skip in reference files into `SKILL.md`. [Haiku smoke runs](../evals/haiku-smoke-2026-09-25.md): 7/10 → 9/10 on the shipped runtime (SHA-256 `e9cc02ad…7f586158`). |
...
|---|---|---|
| [Haiku smoke runs](../evals/haiku-smoke-2026-09-25.md) | Claude Haiku 4.5 on 10 repository cases, scored from files and transcripts: 7/10 on candidate.11, 9/10 on the shipped candidate.12. Stale resume passed in all 3 candidate.12 wordings (2 drafts and final); the release verdict held in 2 of 4 runs on candidate.12, 0 of 3 on candidate.13 (0 of 3 before), 1 of 3 on candidate.15 and 2 of 3 on candidate.16. Candidate.16's delegation reason appeared in 2 of 3 case 03 runs, versus 0 of 3 on candidate.15. | One run per case in the main table, no run without AMC. Haiku still self-approved one candidate.16 release case without independent review. The fan-out case still fails intermittently, and its check is stricter than the skill. |
| Anthropic prompt audit of `SKILL.md` (2026-09-26) | Run by a Sonnet agent with the `claude-api` skill's prompt audit: no stale or duplicated rules. It flagged three idioms in the release gate (the IMPORTANT marker, the numbered steps and the excuses table); all three stay as scoped fixes for a measured small-model failure. | One pass by one agent. It judges wording, not behavior. |
| [candidate.11 readiness review](reviews/readiness-2026-09-24.md) | Full repository audit, targeted engineering checks, exact project install, explicit Codex skill input, checked review repairs and an actual three-file read. | Native read needed one approved command. No unassisted sandbox, desktop picker, other-host behavior or comparative improvement claim. Published via PR #11 (main `2459323`). |
