...
| **0.2.0-candidate.9** | Open PR source ([#6](https://github.com/byensitmagnus/agent-mission-control/pull/6)); not merged; no published ZIP. | Engineering PASS. Nine Codex comparison runs bound in [results](../evals/candidate.9/comparison/results/README.md). Holdout: no unique correctness win; more time and total input. Audit notify-429 was not a valid hidden contract. Behavior, cost and speed remain NOT VERIFIED. |
