...
| **0.2.0-candidate.9** | Open PR source ([#6](https://github.com/byensitmagnus/agent-mission-control/pull/6)); not merged; no published ZIP. | Engineering PASS. Nine Codex runs kept as a [closed postmortem](../evals/candidate.9/README.md), not a research program. No unique correctness win. Behavior, cost and speed remain NOT VERIFIED. [Field state](field-state.md) grades sources A–E. |
