...
|---|---|---|
| Author's local Context Diamond skill (not published here) | Dependency test, minimal context, exclusive ownership, fresh review and owner feedback; adapted in work routing | Useful independent work only; contexts/review add cost. Reject fixed fan-out and mandatory companion ownership. Native agents execute. |
...
| [Microsoft SkillOpt](https://github.com/microsoft/SkillOpt/tree/79124b37e9a6371e13b753f8bcd7adb1e493ade1) and the author's local Sleep skill (not published here) | `evaluation/gate.py`, `skillopt_sleep/cycle.py`, `staging.py`: score selection, train/selection/test separation, source pins and recoverable adoption; adapted in separate learning | Replay and held-out evaluation cost real work. Reject live adoption and instruction-density scoring. Source tests were inspected; execution unavailable here without pytest. No learner installed. |
| The author's local Sleep-Learned skill (not published here) and a locally modified ECC-origin continuous-learning-v2 skill | Scoped observations subordinate to evidence/current instructions; candidate lessons with activation and contradiction risk | No automatic promotion of memory to policy, global hooks or transcript collection. Domain-specific learned rules remain conditional. |
...
| ECC-origin strategic-compact skill as installed on the author's machine | Preserve semantic mission facts before context loss | Native host owns compaction/history. No AMC hook, transcript replica or custom context engine. |
...
not all published. Evidence grades for each source are in the repository's [sources page](https://github.com/byensitmagnus/agent-mission-control/blob/main/docs/sources.md); inspected anchors for the iteration-2 sources are in the historical [design-mining notes](https://github.com/byensitmagnus/agent-mission-control/blob/main/evals/design-mining.md). Historical source comparisons belong to repository development, not the
installed runtime.
