...
| [Microsoft SkillOpt](https://github.com/microsoft/SkillOpt/tree/79124b37e9a6371e13b753f8bcd7adb1e493ade1) and the author's local Sleep skill | `evaluation/gate.py`, `skillopt_sleep/cycle.py`, `staging.py`: score selection, train/selection/test separation, source pins and recoverable adoption; adapted in separate learning | Replay and held-out evaluation cost real work. Reject live adoption and instruction-density scoring. Source tests were inspected; execution unavailable here without pytest. No learner installed. |
| Author's local Sleep-Learned and continuous-learning-v2 skills | Scoped observations subordinate to evidence/current instructions; candidate lessons with activation and contradiction risk | No automatic promotion of memory to policy, global hooks or transcript collection. Domain-specific learned rules remain conditional. |
...
| Author's local strategic-compact skill | Preserve semantic mission facts before context loss | Native host owns compaction/history. No AMC hook, transcript replica or custom context engine. |
...
summarized in the repository's [design-mining notes](https://github.com/byensitmagnus/agent-mission-control/blob/main/evals/design-mining.md). Historical source comparisons belong to repository development, not the
installed runtime.
